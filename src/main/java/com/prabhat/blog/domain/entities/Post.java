...
public class Post {
    ...
    //This  is to connect to the User class for auth related purpose and others
    @ManyToOne(fetch = FetchType.LAZY) //many post can be posted by one author
    @JoinColumn(name="author_id", nullable = false)
    private User author;

    // This is connection to Category class
    @ManyToOne(fetch = FetchType.LAZY) // Many post can belong to one Category
    @JoinColumn(name = "category_id",nullable = false)
    private Category category;

    //This is the connection to Tag class
    @ManyToMany
    @JoinTable(
            name = "post_tags",
